function F = LoadStROIs4(folderName,frame)
...
for i = 1 : size(files,1)
       ...
       for k=fstart:8:frame       
           ...
           [Tmin Tcmin]= min(Txmin);
            ...
            [indexedImage storedColorMap] = gray2ind(grayImage,8);
           ...
           DeltaT =(Tmax-Tmin)./ Tmgv;
           
           for m = 1:1:240 
               for n = 1:1:320
                   if ((Y(m,n,k)<= double(Tmin) + DeltaT * grayImage(m,n)) || (Y(m,n,k)>= double(Tmax)- DeltaT * grayImage(m,n))) 
                     H(m,n) = 0;
                   else H(m,n)=Y(m,n,k);
                 %H(m,n)=Y(m,n,k);
                   end
