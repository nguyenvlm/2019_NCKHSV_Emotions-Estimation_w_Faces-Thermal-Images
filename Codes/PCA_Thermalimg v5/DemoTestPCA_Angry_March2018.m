% DemoTest_PCA_ThermalData_Angry_10Feb2015_Glasses


%folderName = '\\k1200\k1200\s1250203\.windows\Desktop\data\Thermal Database\';
...
for temp=1:1:10
    ...
    for nEmotion = 1:1:7
     ...
     %   emotionFolder = strcat(folderName,emotions{nEmotion},'\');
      ...
      %Faces = LoadThermalFaces(emotionFolder,frame);
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for i = 1:length(imageTests)
        ...
        for nEmotion = 1:1:7
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%
            %recontructedImg  = FindRestoredImageNew(X,weightTrainsClass{nEmotion},avgTrainsClass{nEmotion},ukTrainsClass{nEmotion});
            %%%%%%%%%%%%%%%%%%%%%%%%          
     ...
     %   if i >= imageTestClass{best}(1) && i <= imageTestClass{best}(2)
        ...
        %if (best==1) n_Wronganger = n_Wronganger +1;
     %  else
        if (best==2) n_Wrongdisgust = n_Wrongdisgust +1;
     ...
     %percentageWrongA  = n_Wronganger/length(imageTests)
   ...
   % d=[expMatrix;expWAMatrix;expWHMatrix;expWFMatrix;expWNMatrix;expWSaMatrix;]
    d=[expMatrix;expWDMatrix;expWFMatrix;expWHMatrix;expWNMatrix;expWSaMatrix;expWSuMatrix]
end

    xlswrite('D:\LVThS K27\PCA_Thermalimg v5\Result0.4\anger04.xls',d);
